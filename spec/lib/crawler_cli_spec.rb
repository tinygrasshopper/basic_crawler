...
describe CrawlerCLI do
  it 'should delegate to the crawler manger' do
    ...
    expect(crawler_manager).to receive(:crawl!).and_return(double('result', links: {}))

    CrawlerCLI.crawl! 'http://temp'
  end

  it 'should render the counter output' do
    expect_any_instance_of(CrawlerManager).to receive(:crawl!).and_return(double('result', links: {Link.new('/one.html', nil) => 2, Link.new('/two.html', nil) => 1}))
